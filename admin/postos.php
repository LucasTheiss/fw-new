    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form action="actions/posto_actions.php" method="POST">
                ...
                <div class="form-group">
                    <label for="edit_coordenadas">Coordenadas</label>
                    <input placeholder="Ex: 123.42343, -123.12324" type="text" id="edit_coordenadas" name="coordenadas" required>
                </div>
...
<script>
    ...
    function openEditModal(posto) {
        ...
        document.getElementById('edit_coordenadas').value = posto.latitude + ', ' + posto.longitude;
    ...
    document.getElementById('add_endereco').addEventListener('blur', async function() {
        const elemento = this;
        const inputCoordenadas = document.getElementById('add_coordenadas');
        if (elemento.value.trim() === '') return;
        try {
            const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(elemento.value)}&format=json&limit=1`);
    ...
    document.getElementById('edit_endereco').addEventListener('blur', async function() {
        const elemento = this;
        const inputCoordenadas = document.getElementById('edit_coordenadas');
        if (elemento.value.trim() === '') return;
        try {
            const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(elemento.value)}&format=json&limit=1`);
            const data = await response.json();
